      ...
      <section class="right-panel">
        ...
        <div v-else class="results-wrapper">
          ...
          <div class="card erp-card" v-if="resultData.orderData">
            <h3>📦 訂單與客戶資訊 </h3>
            <div class="erp-grid">
              <div><strong>客戶姓名：</strong> {{ resultData.orderData?.customer?.name || '未填寫名稱' }}
              ({{ resultData.orderData?.customer?.membership_tier || 'Standard' }})</div>
      ...
      for (const block of lines) {
        ...
        if (eventMatch && dataMatch) {
          ...
          if (eventType === 'status') {
            ...
            // 核心修復：拆解兩層 .data，拿到真正的結果物件
            const receivedData = eventData.data?.data || eventData.data;
